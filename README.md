...
- Optional passwordless Start and Stop for administrator accounts after one explicit macOS approval; the administrator-password prompt remains the default and fallback.
...
**Stop** stops every installed known Corplink job and related process. The helper asks the vendor CLI to disconnect VPN and SWG, unloads the user jobs and then the system jobs in bounded batches, terminates only matching orphan processes, restores any original `schg` or `uchg` flags, and requires five uninterrupted seconds with no known restart. Active System Extensions are not uninstalled: when the jobs and processes have stopped but an extension remains enabled, the UI reports a successful runtime stop with a warning instead of an operation failure. While an operation is running, the status card identifies the current phase instead of showing an unexplained spinner.
...
The universal app is created at `dist/Corplink Control.app`. Reading status does not require elevated privileges. By default, starting or stopping uses the standard macOS administrator-password prompt. Passwordless control is optional and must be explicitly enabled under **Settings > Privileged control > Passwordless start and stop**. Enabling it registers an embedded `SMAppService` LaunchDaemon and requires one administrator approval under **System Settings > General > Login Items & Extensions**. After approval, control actions from the current foreground administrator account use a code-signature-checked XPC connection without asking for the password each time. Standard accounts always use the administrator-password prompt. The prompt is also used while the option is off, approval is pending, or the app can prove that an XPC request was not submitted.

Control and status subprocesses have bounded execution times, passwordless XPC requests have a reply deadline, and the administrator-prompt path has a three-minute upper bound. A non-responsive Corplink CLI or helper therefore releases the busy indicator instead of leaving the app spinning forever. If the deadline expires after an XPC request may have been submitted, the app preserves the last displayed state and does not repeat the privileged action automatically.

Passwordless registration is tied to the bundle build number and the exact designated code requirements of both the App and its embedded helper. After an App upgrade or a differently signed local rebuild, the previous helper is never reused silently—even when the bundle build number did not change. Before the next passwordless action, the foreground administrator authenticates once; the App then waits for both Service Management to report the old service unregistered and `launchctl` to confirm that its system job has disappeared before registering the helper from the current bundle. Every XPC action starts with a bounded read-only probe that must return the current protocol and build; only then is the control request considered submitted. Cancellation sends no control request, and pre-submission migration or probe failure safely falls back to the administrator-password path.

For reliable LaunchDaemon startup, keep the app in `/Applications`. Both XPC peers enforce the exact designated requirement read from the signed app bundle, including the exact cdhash for local ad-hoc builds. The helper accepts only fixed action names, requires the caller to be both the foreground console user and a member of macOS's administrator group, serializes privileged operations, and runs those actions in-process rather than executing a replaceable helper path. Builds use Hardened Runtime. Public distribution should additionally use Developer ID signing and notarization; set `CORPLINK_CODE_SIGN_IDENTITY` to the signing identity when building.

When testing a local build, quit Corplink Control, replace the existing `/Applications/Corplink Control.app` with the newly built app, and launch that installed copy. The app deliberately refuses to register its privileged helper while running from Downloads, the build directory, or another location. Confirm that the About page reports version 1.6.1 (build 11) before testing helper approval. Then enable **Settings > Privileged control > Passwordless start and stop**; control buttons never register the helper implicitly, and they continue to work through the password prompt when the option is off. Every transition from off to on requires the foreground account to be an administrator and creates a fresh macOS device-owner authentication context, even if macOS retained its underlying Background Item approval from an earlier registration.
